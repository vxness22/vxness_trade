// Single source of truth for "is this instrument tradable right now?"
//
// Rule:
...
//
// Mirrored on the frontend at frontend/src/utils/marketHours.js — keep both in sync.
...
const WEEKEND_OPEN_EXTRA = new Set([
  // e.g. 'BTCUSD' is already covered by the crypto classifier below.
])
...
// True when `now` (default: real time) falls on the FX weekend in UTC.
...
export function marketClosedReason(symbol, now = new Date()) {
  ...
  return 'Market is closed on weekends for this instrument.'
}
