    ...
    try {
      if (user && user.email) {
        ...
        const emailResult = await sendTemplateEmail('kyc_approved', user.email, {
          ...
          email: user.email,
          documentType: kyc.documentType,
          approvedAt: new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'medium', timeStyle: 'short' }),
          platformName: settings?.fromName || 'vxness',
    ...
    try {
      ...
      if (user && user.email) {
        ...
        await sendTemplateEmail('kyc_rejected', user.email, {
          ...
          documentType: kyc.documentType,
          rejectionReason: kyc.rejectionReason,
          rejectedAt: new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'medium', timeStyle: 'short' }),
          platformName: settings?.fromName || 'vxness',
          loginUrl: settings?.loginUrl || 'https://vxness.in/user/login',
          supportEmail: settings?.fromEmail || 'support@vxness.in',
          year: new Date().getFullYear().toString()
