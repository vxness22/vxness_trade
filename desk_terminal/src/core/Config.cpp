...
//
// This terminal shares a code lineage with other brokers' desktop builds, and
// those earlier steps DID carry the config forward, because each was a rename
// of the same broker: same account, same backend, only the host moved. Vxness
// is a *different platform*. Another broker's token or API key authenticates
// nothing here, and adopting one would also drag its endpoints in — pointing
// this build at that broker's API. So a Vxness install starts with a clean
// config and a real sign-in.
//
// For the same reason nothing deletes those files either: another broker's
// terminal may still be installed on this machine and its config is its own.
// The two coexist — different config path, different installer AppId.
