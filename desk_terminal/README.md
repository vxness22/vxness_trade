...
The WebSocket must use the `api.` host — `vxness.in` proxies REST but
...
> This build shares a code lineage with other brokers' desktop terminals but
> no identity: different config path, different installer AppId, and **no**
> config migration. Another broker's sign-in is not carried over — that account
> does not exist on Vxness's backend. The apps install side by side.
