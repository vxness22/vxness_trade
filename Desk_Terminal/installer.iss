...
[Setup]
; A FRESH AppId. This build was forked from the TuskaEx terminal, which was
; itself a Bull4x → SwissCresta lineage; keeping TuskaEx's AppId would make the
; Vxness installer overwrite an existing TuskaEx install and take over its
; Apps & features entry, on a machine whose owner may still trade there.
;
