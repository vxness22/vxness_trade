# Plan F — Vxness Redesign: Auth Screens (Phase 6)

> Subagent-driven execution.

**Goal:** Replace PipHigh-era Login / Signup / ForgotPassword with Vxness-styled versions, plus a new 2FA TOTP screen. Wire AuthContext to handle the 2FA challenge response.
...
const styles = StyleSheet.create({
  ...
  tagline: { color: vx.textMuted, fontFamily, fontSize: sizes.body, letterSpacing: 2, textTransform: 'uppercase' },
  ...
  return (
    <Screen edges={['top','bottom']}>
      <View style={styles.headerRow}>
        <IconButton icon={<Ionicons name="chevron-back" size={22} color={vx.textPrimary} />} accessibilityLabel="Back" onPress={() => step === 1 ? navigation.goBack() : setStep(step - 1)} />
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
          <View style={styles.stepper}>
            {[1,2,3].map((s) => (
              <View key={s} style={[styles.stepDot, s <= step && { backgroundColor: vx.accent }]} />
            ))}
          </View>
          ...
          {step === 1 ? (
            ...
            <>
              ...
              <TextInput value={otp} onChangeText={setOtp} placeholder="123456" placeholderTextColor={vx.textMuted} keyboardType="number-pad" maxLength={6} autoCorrect={false} style={[styles.input, styles.codeInput]} />
            ...
            <>
              ...
              <TextInput value={newPwd} onChangeText={setNewPwd} placeholder="••••••••" placeholderTextColor={vx.textMuted} secureTextEntry autoCapitalize="none" autoCorrect={false} style={styles.input} />
              <Text style={[styles.label, { marginTop: space.md }]}>Confirm Password</Text>
              <TextInput value={confirmPwd} onChangeText={setConfirmPwd} placeholder="••••••••" placeholderTextColor={vx.textMuted} secureTextEntry autoCapitalize="none" autoCorrect={false} style={styles.input} />
...
const styles = StyleSheet.create({
  ...
  stepDot: { width: 30, height: 6, borderRadius: 3, backgroundColor: vx.bgPressed },
...
- Log out (or clear app data) → Login screen shows Vxness wordmark + email + password (eye toggle) + Forgot link + big orange Log In + Sign up link.
